...
void imageManager::setImageReSize(string strKey, int x, int y)
{
	image* img = findImage(strKey);
...
void imageManager::minimapRender(string strKey, HDC mapHdc, int destX, int destY, int width, int height, HDC sourHdc, int startX, int startY, int endX, int endY)
{
	//이미지를 찾은 후 렌더
	image* img = findImage(strKey);
	if (img) img->minimapRender(mapHdc, destX, destY, width, height, sourHdc, startX, startY, endX, endY);
}

void imageManager::setColorImage(string strKey, COLORREF setColor, COLORREF transColor)
{
	image* img = findImage(strKey);
	if (img) img->setColorImage(setColor, transColor);
