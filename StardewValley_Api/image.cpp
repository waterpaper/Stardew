...
void image::alphaFrameRender(HDC hdc, int destX, int destY, BYTE alpha)
{
	...
	else
	{
		AlphaBlend(hdc, destX, destY, _imageInfo->frameWidth, _imageInfo->frameHeight, 
			_imageInfo->hMemDC, _imageInfo->currentFrameX * _imageInfo->frameWidth, _imageInfo->currentFrameY * _imageInfo->frameHeight, _imageInfo->currentFrameX * _imageInfo->frameWidth, _imageInfo->currentFrameY * _imageInfo->frameHeight, _blendFunc);
	}
}
...
void image::alphaFrameRender(HDC hdc, int destX, int destY, int frameX, int frameY, BYTE alpha)
{
	...
	{
		AlphaBlend(hdc, destX, destY, _imageInfo->frameWidth, _imageInfo->frameHeight,
			_imageInfo->hMemDC, frameX * _imageInfo->frameWidth, frameY * _imageInfo->frameHeight, frameX * _imageInfo->frameWidth, frameY * _imageInfo->frameHeight, _blendFunc);
	}
}
...
void image::minimapRender(HDC mapHdc, int destX, int destY, int width, int height, HDC sourHdc, int startX, int startY, int endX, int endY)
{
	StretchBlt(mapHdc, destX, destY, width, height, sourHdc, startX, startY, endX, endY, SRCCOPY);
}

void image::setColorImage(COLORREF setColor, COLORREF transColor)
{
	for (int i = 0; i <  _imageInfo->height; i++)
	{
		for (int j = 0; j < _imageInfo->width ; j++)
		{
			COLORREF rgbTemp = GetPixel(_imageInfo->hMemDC, j, i);

			if (rgbTemp == transColor|| rgbTemp == RGB(0,0,0)) continue;

			rgbTemp = RGB((GetRValue(rgbTemp)+ GetRValue(setColor))/2, (GetGValue(rgbTemp)+ GetGValue(setColor))/2, (GetBValue(rgbTemp)+ GetBValue(setColor))/2);
			
			SetPixel(_imageInfo->hMemDC, j, i, rgbTemp);
		}
	}
}
